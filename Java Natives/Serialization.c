...
/*
* Class:     com_troy_serialization_io_NativeOutput
* Method:    ngetBuffer
* Signature: (JI)[B
*/
...
JNIEXPORT j##type JNICALL Java_com_troy_serialization_util_NativeUtils_##type##sToFWrite(JNIEnv * env, jclass class, long fd, j##type##Array srcJ, jint srcOffset, jint elements, jboolean swapEndianess) {\
	if (fd == 0 || srcJ == NULL) {											\
		return INVALID_ARGUMENT;											\
	}																		\
	j##type* src = (*env)->GetPrimitiveArrayCritical(env, srcJ, NULL);		\
...
//Methods for manipulating java arrays
#define xToNative(type, capitalType) \
JNIEXPORT jint JNICALL Java_com_troy_serialization_util_NativeUtils_##type##sToNative(JNIEnv * env, jclass class, jlong dest, j##type##Array src, jint offset, jint elements) {\
	if(src == 0 || dest == 0) return INVALID_ARGUMENT;\
	(*env)->Get##capitalType##ArrayRegion(env, src, offset, elements, (j##type *)dest); \
	return 0;\
}
...
xToNative(byte, Byte)
xToNative(short, Short)
xToNative(int, Int)
xToNative(long, Long)
xToNative(float, Float)
xToNative(double, Double)
xToNative(char, Char)
...
/*
JNIEXPORT jint JNICALL Java_com_troy_serialization_FourBitEncodingCharset_nEncode
(JNIEnv * env, jobject charset, jcharArray srcJ, jbyteArray destJ, jint srcOffset, jint destOffset, jsize srcLength, jlong errorAddress, jboolean checkForErrors)
{
	...
	int i = 0;
	while (i < srcLength) {
		if (soFar == -1) {//Start of a new byte
			if (i + 1 == srcLength) {// If this is the last byte
				dest[bytesWritten++] = (jbyte)(FOUR_BIT_ENCODING_CACHE[src[i]] << 4);
				// This will only happen if we are writing an odd number of characters
				break;
			}
			else {
				soFar = (jbyte) (FOUR_BIT_ENCODING_CACHE[src[i]] << 4);
			}
		} else {
			soFar |= FOUR_BIT_ENCODING_CACHE[src[i]];
			dest[bytesWritten++] = soFar;
			soFar = -1;
		}
		i++;
	}
...
#endif
>>>>>>> branch 'master' of https://github.com/TroyNeubauer/Troy-Serialization.git
