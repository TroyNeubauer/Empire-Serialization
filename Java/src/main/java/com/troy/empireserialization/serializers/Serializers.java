...
public class Serializers {
	...
	/**
	 * Fetches and returns the serializer associated with the desired type, or creates a new serializer if there is
	 * currently no mapping associated with requested type.
	 * 
	 * @return The serializer associated with the given type
	 */
	public static <T> Serializer<T> getSerializer(Class<T> type, ClassIDProvider provider) {
		Serializer<T> ser = (Serializer<T>) map.get(type);
		if (ser == null) {
			ser = createSerializer(type, provider);
			map.put(type, ser);
		}
		return ser;
	}
	...
	/**
	 * Registers the given type to be serialized using the desired serializer. Returns {@code true} if calling this
	 * method overrode a previous type - serializer entry.
	 * 
	 * @param type
	...
	private static <T> Serializer<T> createSerializer(Class<T> type, ClassIDProvider provider) {
		if (!SerializationUtils.DYNAMIC_CLASS_LOADING_ENABLED) {// Use the default one if we can't create a dynamic one
			return new FieldSerializer<T>(type, provider);
